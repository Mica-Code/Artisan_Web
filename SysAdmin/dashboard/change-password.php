...
if(isset($_POST['submit'])){
	...
	if(empty($error)){
		...
		$query = "SELECT * from admin_user WHERE admin_id = $session_id";
		...
		elseif($new_password != $c_new_password){
			...
			$query = "UPDATE admin_user SET admin_pass='$hashpassword' WHERE admin_id=$session_id" or die(mysqli_error($dbc));;
			$result = mysqli_query($dbc, $query);
			if($result){
